...
def get_src_file_path(src_filename, use_sample_files=False):
    """

    :param src_filename: le nom du fichier voulu dans  DATA_SRC_DIR
    :param use_sample_files: permet d'utiliser les fichiers "sample" fournis sans avoir téléchargé les versions les plus récentes des fichiers
     OpenData AN (utilisé pour les tests et expérimentations)
    :return:
    """
...
def get_out_dir(base_subdir_name=None):
    out_dir_path = os.path.join(SCRIPT_ROOT_DIR, "out_dir", base_subdir_name if base_subdir_name else "")
    if not os.path.exists(out_dir_path):
        os.mkdir(out_dir_path)
    return out_dir_path


def get_out_file_name(filename, base_subdir_name=None):
    return os.path.join(get_out_dir(base_subdir_name=base_subdir_name), filename)
