...
Son usage et son extention pour de nouvelles présentation ou de nouveau fragments de XML, devrait être 'simple'.
...
Si vous voulez ajouter une nouvelle présentation pour un type de fragment XML, vous n'avez qu'a ajouter une  méthode
 ...
 FormatXXXX ou XXXX est le nom du tag racine de l'élément que vous souhaitez formater.
 Ensuite ajoutez simplement une méthode short_form, statique, et autant de méthodes xxx_form que de besoin.
"""

from common_XPath import UID
from common_XPath import ACTEUR_IDENT_ELEM, ACTEUR_DATNAIS_ELEM
from common_XPath import ORGANE_TYPE_ORGANE, ORGANE_LIBELLE
from common_XPath import MANDAT_TYPE_ORGANE, MANDAT_TYPE_MANDAT, MANDAT_ORGANE_REF, MANDAT_DATEDEBUT, MANDAT_DATEFIN
...
class FormatActeur(FormatBaseKlass):
    ...
    def long_form(acteur_xml):
        (uid,) = UID(acteur_xml)
        ident = ACTEUR_IDENT_ELEM(acteur_xml)[0].getchildren()
        (date_naiss,) = ACTEUR_DATNAIS_ELEM(acteur_xml)
